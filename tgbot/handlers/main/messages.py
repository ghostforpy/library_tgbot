...
# Стартовый диалог
WELCOME = (
    "Добрый день! Вы находитесь в Telegram боте. \n"
    "Используйте кнопки меню для работы с ботом."
)
AFFILATE_MESS = (
    "Если вы хотите видеть своего коллегу/партнера/друга в нашем сообществе "
...
def get_start_mess(user: User):
    ...
    if not user.username:
        res += (
            "\n<b>"
            + _(
                "У Вас в Телеграм не введено имя пользователя. "
                "Это может затруднить общение с вами. Введите его в настройках Телеграм"
            )
            + "</b>"
        )
...
NO_ADMIN_GROUP = (
    'Группа "Администраторы" не найдена или не указан ее чат. Сообщите об ошибке'
)
